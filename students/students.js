...
function displayStudents() {
  ...
  studentsList.slice(start, end).forEach((student) => {
    ...
    row.innerHTML = `
      ...
      <td><img class="main__table_statusImg" src="../assets/${student.status ? "online.svg" : "offline.svg"}" alt="Status"/></td>
      <td>
...
function attachRowListeners() {
  ...
  document.querySelectorAll(".main__table_selectStudent").forEach((checkbox) => {
    checkbox.addEventListener("change", (e) => {
      if (!e.target.checked) {
        ...
        Array.from(document.querySelectorAll(".main__table_selectStudent")).every((c) => c.checked)
      ) {
  ...
  document.querySelectorAll(".edit-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      studentToEdit = { id };
      document.getElementById("modal__addEdit_heading").textContent = "Edit student";
      document.getElementById("modal__addEdit_GroupInput").value = row.cells[1].textContent;
      ...
      document.getElementById("modal__addEdit_GenderInput").value = row.cells[3].textContent;
      document.getElementById("modal__addEdit_BirthdayInput").value = row.cells[4].textContent;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("modal__delete_confirmBtn").addEventListener("click", async (e) => {
  ...
  document.getElementById("modal__delete_cancelBtn").addEventListener("click", () => {
    document.getElementById("modal__delete_container").classList.remove("active");
  });

  document.getElementById("modal__delete_closeBtn").addEventListener("click", () => {
    document.getElementById("modal__delete_container").classList.remove("active");
  });

  // Modal Add/Edit Listeners
  document.getElementById("main__addStudentBtn").addEventListener("click", () => {
    document
    ...
    document.getElementById("modal__addEdit_heading").textContent = "Add new student";
    studentToEdit = null;
    document.getElementById("modal__addEdit_container").classList.add("active");
  });
  ...
  document.getElementById("modal__addEdit_form").addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_API_URL}/students${studentToEdit ? `/${studentToEdit.id}` : ""}`,
        {
          method,
          headers: { "Content-Type": "application/json" },
  ...
  document.getElementById("modal__addEdit_cancelBtn").addEventListener("click", () => {
    document.getElementById("modal__addEdit_container").classList.remove("active");
    studentToEdit = null;
  });

  document.getElementById("modal__addEdit_closeBtn").addEventListener("click", () => {
    document.getElementById("modal__addEdit_container").classList.remove("active");
    studentToEdit = null;
  });
});